...
int UNUSED CalcBaseDamage(void *bw, struct BattleStruct *sp, int moveno, u32 side_cond UNUSED,
    ...
    for (i = 0; i < maxBattlers; i++) {
        ...
        // if Aura Break is present but not Dark Aura
        if (!darkAuraApplied
            && (movetype == TYPE_DARK)
            && (GetBattlerAbility(sp, sp->rawSpeedNonRNGClientOrder[i]) == ABILITY_AURA_BREAK)
            && (CheckSideAbility(bw, sp, CHECK_ABILITY_ALL_HP, 0, ABILITY_DARK_AURA) == FALSE)) {
                darkAuraApplied = FALSE;
                basePowerModifier = QMul_RoundUp(basePowerModifier, UQ412__0_75);
                continue;
        }
        ...
        // if Aura Break is present but not Fairy Aura
        if (!fairyAuraApplied
            && (movetype == TYPE_DARK)
            && (GetBattlerAbility(sp, sp->rawSpeedNonRNGClientOrder[i]) == ABILITY_AURA_BREAK)
            && (CheckSideAbility(bw, sp, CHECK_ABILITY_ALL_HP, 0, ABILITY_FAIRY_AURA) == FALSE)) {
            fairyAuraApplied = FALSE;
            basePowerModifier = QMul_RoundUp(basePowerModifier, UQ412__0_75);
            continue;
        }
#endif
